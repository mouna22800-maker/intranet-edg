...
export default function DepartmentHomeView({ department, localArticles, onNavigate, currentUser }: DepartmentHomeViewProps) {
  ...
  return (
    <div id={`dept-home-${department.code}`} className="space-y-10 pb-16 font-sans">
      ...
      {/* 2.5 Dynamic Value Proposition Banner */}
      {department.valueKey && (
        <div className={`border border-l-4 rounded-xl p-5 flex items-start space-x-4 shadow-sm transition-all duration-350 ${theme.badgePill}`}>
          <div className="p-2 bg-slate-800 dark:bg-slate-700 text-white rounded-lg shrink-0 mt-0.5 shadow-sm">
            <LucideIcon name="Award" size={18} />
          </div>
          <div className="space-y-1">
            <span className={`text-[10px] uppercase font-mono font-bold tracking-wider block ${theme.textPrimary}`}>
              Valeur institutionnelle phare
            </span>
            <h4 className="font-display font-black text-sm text-slate-900 dark:text-white uppercase">
              {department.valueKey}
            </h4>
            <div
              className="rich-content text-xs text-slate-650 dark:text-slate-300 leading-relaxed font-sans font-medium"
      ...
      {/* 3. Splitted rows: Welcome message vs. local articles */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        
        {/* Left column: Word from Director */}
        <div className="lg:col-span-12 xl:col-span-5 bg-white dark:bg-[#0c0c0e] border border-zinc-200/50 dark:border-zinc-800 rounded-xl p-6 sm:p-8 space-y-6 shadow-sm transition-all duration-300">
        ...
        <div className="lg:col-span-12 xl:col-span-7 space-y-4">
          <div className="flex items-center justify-between border-b border-slate-205 dark:border-white/10 pb-3">
      ...
      {/* 4. Nos Projets Stratégiques (Données réelles issues de la table mysql 'projet') */}
      <section className="bg-white dark:bg-[#0c0c0e] border border-zinc-200/60 dark:border-zinc-800 rounded-xl p-6 sm:p-8 space-y-6 shadow-sm transition-all duration-300">
      ...
      {/* 5. Contact & Destinataires Directs (Données réelles issues de la table 'recipient' et 'contact') */}
      <section className="bg-white dark:bg-[#0c0c0e] border border-zinc-200/60 dark:border-zinc-800 rounded-xl p-6 sm:p-8 space-y-6 shadow-sm transition-all duration-300">
        <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-white/5">
          <div className="flex items-center space-x-2">
        ...
        {loadingContacts ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {contacts.map((rec: any, idx: number) => (
              <div key={idx} className="p-5 border border-zinc-200/60 dark:border-zinc-800/80 rounded-xl space-y-4 bg-zinc-50/40 dark:bg-zinc-900/30">
